...
{
  domain?((void )0) : __assert_fail("domain","linkCells.c",85,__PRETTY_FUNCTION__);
...
// local number of boxes
  ...
  ll -> nHaloBoxes = 2 * ((ll -> gridSize[0] + 2) * (ll -> gridSize[1] + ll -> gridSize[2] + 2) + ll -> gridSize[1] * ll -> gridSize[2]);
  ...
  for (int iBox = 0; iBox < ll -> nTotalBoxes; ++iBox) 
    ll -> nAtoms[iBox] = 0;
  ll -> gridSize[0] >= 2 && ll -> gridSize[1] >= 2 && ll -> gridSize[2] >= 2?((void )0) : __assert_fail("(ll->gridSize[0] >= 2) && (ll->gridSize[1] >= 2) && (ll->gridSize[2] >= 2)","linkCells.c",109,__PRETTY_FUNCTION__);
  return ll;
}
...
// Halo in Z+
  if (iz == gridSize[2]) {
    iBox = boxes -> nLocalBoxes + 2 * gridSize[2] * gridSize[1] + 2 * gridSize[2] * (gridSize[0] + 2) + (gridSize[0] + 2) * (gridSize[1] + 2) + (gridSize[0] + 2) * (iy + 1) + (ix + 1);
  }
  else {
// Halo in Z-
    if (iz == -1) {
      iBox = boxes -> nLocalBoxes + 2 * gridSize[2] * gridSize[1] + 2 * gridSize[2] * (gridSize[0] + 2) + (gridSize[0] + 2) * (iy + 1) + (ix + 1);
    }
    else {
// Halo in Y+
      if (iy == gridSize[1]) {
        iBox = boxes -> nLocalBoxes + 2 * gridSize[2] * gridSize[1] + gridSize[2] * (gridSize[0] + 2) + (gridSize[0] + 2) * iz + (ix + 1);
      }
      else {
// Halo in Y-
        if (iy == -1) {
...
// local link celll.
{
  ...
  iBox >= 0?((void )0) : __assert_fail("iBox >= 0","linkCells.c",233,__PRETTY_FUNCTION__);
  iBox < boxes -> nTotalBoxes?((void )0) : __assert_fail("iBox < boxes->nTotalBoxes","linkCells.c",234,__PRETTY_FUNCTION__);
  return iBox;
}
...
void moveAtom(LinkCell *boxes,Atoms *atoms,int iId,int iBox,int jBox)
{
  ...
  boxes -> nAtoms[jBox]++;
  boxes -> nAtoms[jBox] < 64?((void )0) : __assert_fail("boxes->nAtoms[jBox] < 64","linkCells.c",249,__PRETTY_FUNCTION__);
  boxes -> nAtoms[iBox]--;
...
// It's a halo box
{
    ...
    if (ink < 2 * gridSize[1] * gridSize[2]) {
      ...
      if (ink < 2 * gridSize[2] * (gridSize[1] + gridSize[0] + 2)) {
