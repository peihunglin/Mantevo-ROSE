...
c       stop ' stop: Before call viscterms in step.f/step'
      ...
      if (ivis.eq.1) then
            call viscterms(jmax,kmax,q,dq,rtxy,dj,s,vnut,btc,bjm,
     &         bjp,bkm,bkp
     &         )
      endif
...
40    continue
      ...
      if (DEBUG.and.nodeid.eq.0) then
       ...
       print*, ' jmaxx,kmaxx = ',jmaxx,kmaxx
       do 10 j=1,jmaxx
       do 10 k=1,kmaxx
        write(*,20) nodeid,nt,j,k,s(j,k,1),s(j,k,2),s(j,k,3),dj(j,k)
20      format(i4,i5,2i3,1p4e15.7)
10     continue
      endif

      irmax = BLAS_IAMAX( 3*jmax*kmax, dq, 1 )
...
c send resmax0 result to all nodes
     ...
     & MPI_COMM_WORLD,ierr)
c
      idmax = BLAS_IAMAX( jmax*kmax, dq, 1 )
      k = (idmax-1)/jmax + 1
      j = idmax - (k-1)*jmax
