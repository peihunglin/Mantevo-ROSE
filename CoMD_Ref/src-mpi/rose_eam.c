...
static void eamDestroy(BasePotential **pPot);
...
static void destroyInterpolationObject(InterpolationObject **a);
...
BasePotential *initEamPot(const char *dir,const char *file,const char *type)
{
  ...
  pot?((void )0) : __assert_fail("pot","eam.c",137,__PRETTY_FUNCTION__);
  pot -> force = eamForce;
  pot -> print = eamPrint;
...
static int eamForce(SimFlat *s)
{
  ...
  pot?((void )0) : __assert_fail("pot","eam.c",181,__PRETTY_FUNCTION__);
...
// save derivative for halo exchange
...
static void eamBcastPotential(EamPotential *pot)
{
  pot?((void )0) : __assert_fail("pot","eam.c",372,__PRETTY_FUNCTION__);
  struct {
  real_t cutoff;
  real_t mass;
  real_t lat;
...
static InterpolationObject *initInterpolationObject(int n,real_t x0,real_t dx,real_t *data)
{
  ...
  table?((void )0) : __assert_fail("table","eam.c",420,__PRETTY_FUNCTION__);
  table -> values = ((real_t *)(comdCalloc(1,(n + 3) * sizeof(real_t ))));
  table -> values?((void )0) : __assert_fail("table->values","eam.c",423,__PRETTY_FUNCTION__);
  table -> values++;
  table -> n = n;
  table -> invDx = 1.0 / dx;
  table -> x0 = x0;
...
static void bcastInterpolationObject(InterpolationObject **table)
{
  ...
  bcastParallel((&buf),(sizeof(buf)),0);
  if (getMyRank() != 0) {
     *table == ((void *)0)?((void )0) : __assert_fail("*table == ((void *)0)","eam.c",525,__PRETTY_FUNCTION__);
...
// Linear interpolation to get phi[0].
