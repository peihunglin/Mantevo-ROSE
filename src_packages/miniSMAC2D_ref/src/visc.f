...
c
c**************************************************************
      subroutine viscterms(jmax,kmax,q,dq,rtxy,dj,s,vnut,
     &                 btc,bjm,bjp,bkm,bkp)
c**************************************************************
...
#include "mpi_params.f"
      ...
      __REAL q(jmax,kmax,3),dq(jmax,kmax,3),rtxy(jmax,kmax,2,3),
...
c DWB 082213: add some smoothing
      if (dcoef2.gt.0.0.or.dcoef4.gt.0.0) then
      do 400 k=2,kmax-1
      do 400 j=2,jmax-1
      do 400 nvar=1,3
       if (k.eq.2.or.j.eq.2.or.k.eq.kmax-1.or.j.eq.jmax-1) then
c 2nd order smoothing near the boundaries
         dq_damp = dq(j+1,k,nvar) + dq(j-1,k,nvar) 
     &      + dq(j,k+1,nvar) + dq(j,k-1,nvar) - 4.0d0*dq(j,k,nvar) 
         s(j,k,nvar) = s(j,k,nvar) + dcoef2*dq_damp 
       else
c 4th order smoothing in the interior
         dq_damp_j = dq(j+2,k,nvar) - 4.0d0*dq(j+1,k,nvar) 
     &      + 6.0d0*dq(j,k,nvar) - 4.0d0*dq(j-1,k,nvar)
     &      + dq(j-2,k,nvar)
         dq_damp_k = dq(j,k+2,nvar) - 4.0d0*dq(j,k+1,nvar) 
     &      + 6.0d0*dq(j,k,nvar) - 4.0d0*dq(j,k-1,nvar)
     &      + dq(j,k-2,nvar)  
         s(j,k,nvar) = s(j,k,nvar)
     &    + dcoef4/8.0d0*(dq_damp_j + dq_damp_k)
       endif
400   continue
      endif
