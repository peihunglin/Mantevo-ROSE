...
void pdv_kernel_c_(int *g_mic_device,int *prdct,
                ...
                double *volume_change)
{
  ...
  int j,k;
  __declspec(target(mic)) double recip_volume,energy_change,min_cell_volume,right_flux,left_flux,top_flux,bottom_flux,total_flux;
...
#pragma offload target(mic:*g_mic_device) \
   ...
   in(energy0      :length(0) alloc_if(0) free_if(0)) \
   ...
   in(volume_change:length(0) alloc_if(0) free_if(0))
