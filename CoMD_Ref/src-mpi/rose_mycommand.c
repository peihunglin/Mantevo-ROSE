...
// add arguments for processing.  Please update the html documentation too!
  ...
  addArg("potDir",100,1,'s',cmd . potDir,(sizeof(cmd . potDir)),"potential directory");
  ...
  addArg("dt",'D',1,100,(&cmd . dt),0,"time step (in fs)");
  addArg("lat",'l',1,100,(&cmd . lat),0,"lattice parameter (Angstroms)");
  addArg("temp",'T',1,100,(&cmd . temperature),0,"initial temperature (K)");
  addArg("delta",'r',1,100,(&cmd . initialDelta),0,"initial delta (Angstroms)");
...
void printCmdYaml(FILE *file,Command *cmd)
{
  ...
  fprintf(file,"Command Line Parameters:\n  doeam: %d\n  potDir: %s\n  potName: %s\n  potType: %s\n  nx: %d\n  ny: %d\n  nz: %d\n  xproc: %d\n  yproc: %d\n  zproc: %d\n  Lattice constant: %g Angstroms\n  nSteps: %d\n  printRate: %d\n  Time step: %g fs\n  Initial Temperature: %g K\n  Initial Delta: %g Angstroms\n\n",cmd -> doeam,cmd -> potDir,cmd -> potName,cmd -> potType,cmd -> nx,cmd -> ny,cmd -> nz,cmd -> xproc,cmd -> yproc,cmd -> zproc,cmd -> lat,cmd -> nSteps,cmd -> printRate,cmd -> dt,cmd -> temperature,cmd -> initialDelta);
  fflush(file);
}
