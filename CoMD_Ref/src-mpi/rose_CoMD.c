...
static Validate *initValidate(SimFlat *sim);
...
static BasePotential *initPotential(int doeam,const char *potDir,const char *potName,const char *potType)
{
  ...
  pot?((void )0) : __assert_fail("pot","CoMD.c",264,__PRETTY_FUNCTION__);
  return pot;
}
...
// Only rank 0 prints
  ...
  fprintf(file,"  Local boxes        : %6d,%6d,%6d = %8d\n",s -> boxes -> gridSize[0],s -> boxes -> gridSize[1],s -> boxes -> gridSize[2],s -> boxes -> gridSize[0] * s -> boxes -> gridSize[1] * s -> boxes -> gridSize[2]);
  fprintf(file,"  Box size           : [ %14.10f, %14.10f, %14.10f ]\n",s -> boxes -> boxSize[0],s -> boxes -> boxSize[1],s -> boxes -> boxSize[2]);
  fprintf(file,"  Box factor         : [ %14.10f, %14.10f, %14.10f ] \n",s -> boxes -> boxSize[0] / s -> pot -> cutoff,s -> boxes -> boxSize[1] / s -> pot -> cutoff,s -> boxes -> boxSize[2] / s -> pot -> cutoff);
...
// sanity checks.  That should not be possible.
  checkCode == failCode?((void )0) : __assert_fail("checkCode == failCode","CoMD.c",481,__PRETTY_FUNCTION__);
  if (failCode != 0) {
    exit(failCode);
  }
}
