...
enum HaloFaceOrder {HALO_X_MINUS=0,HALO_X_PLUS=1,HALO_Y_MINUS=2,HALO_Y_PLUS=3,HALO_Z_MINUS=4,HALO_Z_PLUS=5} ;
/// Don't change the order of the axes in this enum.
enum HaloAxisOrder {HALO_X_AXIS=0,HALO_Y_AXIS=1,HALO_Z_AXIS=2} ;
...
real_t *pbcFactor[6];}AtomExchangeParms;
...
int *recvCells[6];}ForceExchangeParms;
...
static int *mkAtomCellList(LinkCell *boxes,enum HaloFaceOrder iFace,const int nCells)
{
  ...
  count == nCells?((void )0) : __assert_fail("count == nCells","haloExchange.c",350,__PRETTY_FUNCTION__);
  return list;
}
...
static void unloadAtomsBuffer(void *vparms,void *data,int face,int bufSize,char *charBuf)
{
  ...
  bufSize % sizeof(AtomMsg ) == 0?((void )0) : __assert_fail("bufSize % sizeof(AtomMsg) == 0","haloExchange.c",412,__PRETTY_FUNCTION__);
    ...
    case HALO_Z_PLUS:
{
    ...
    default:
    1 == 0?((void )0) : __assert_fail("1==0","haloExchange.c",475,__PRETTY_FUNCTION__);
  }
  int count = 0;
  for (int ix = xBegin; ix < xEnd; ++ix) 
    for (int iy = yBegin; iy < yEnd; ++iy) 
      for (int iz = zBegin; iz < zEnd; ++iz) 
        list[count++] = getBoxFromTuple(boxes,ix,iy,iz);
  count == nCells?((void )0) : __assert_fail("count == nCells","haloExchange.c",484,__PRETTY_FUNCTION__);
  return list;
}
    ...
    case HALO_Z_PLUS:
{
    ...
    default:
    1 == 0?((void )0) : __assert_fail("1==0","haloExchange.c",524,__PRETTY_FUNCTION__);
  }
  int count = 0;
  for (int ix = xBegin; ix < xEnd; ++ix) 
    for (int iy = yBegin; iy < yEnd; ++iy) 
      for (int iz = zBegin; iz < zEnd; ++iz) 
        list[count++] = getBoxFromTuple(boxes,ix,iy,iz);
  count == nCells?((void )0) : __assert_fail("count == nCells","haloExchange.c",533,__PRETTY_FUNCTION__);
  return list;
}
...
static void unloadForceBuffer(void *vparms,void *vdata,int face,int bufSize,char *charBuf)
{
  ...
  ForceExchangeData *data = (ForceExchangeData *)vdata;
  ForceMsg *buf = (ForceMsg *)charBuf;
  bufSize % sizeof(ForceMsg ) == 0?((void )0) : __assert_fail("bufSize % sizeof(ForceMsg) == 0","haloExchange.c",576,__PRETTY_FUNCTION__);
  int nCells = parms -> nCells[face];
  ...
  iBuf == bufSize / sizeof(ForceMsg )?((void )0) : __assert_fail("iBuf == bufSize/ sizeof(ForceMsg)","haloExchange.c",591,__PRETTY_FUNCTION__);
}
...
static int sortAtomsById(const void *a,const void *b)
{
  ...
  aId != bId?((void )0) : __assert_fail("aId != bId","haloExchange.c",656,__PRETTY_FUNCTION__);
  if (aId < bId) {
    return -1;
  }
  return 1;
