...
SUBROUTINE PdV_kernel(g_mic_device,predict,                                          &
...
!DIR$ ATTRIBUTES OFFLOAD:MIC :: recip_volume,energy_change,min_cell_volume,right_flux,left_flux,top_flux,bottom_flux,total_flux
...
!DIR$   in(volume_change:length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   IN(predict)
