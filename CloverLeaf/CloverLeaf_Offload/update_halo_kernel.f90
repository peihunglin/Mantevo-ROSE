...
CONTAINS
  ...
  SUBROUTINE update_halo_kernel(g_mic_device,x_min,x_max,y_min,y_max,                            &
...
!DIR$   in(energy0      :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(pressure     :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(viscosity    :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(soundspeed   :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(density1     :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(energy1      :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(xvel0        :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(yvel0        :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(xvel1        :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(yvel1        :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(vol_flux_x   :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(mass_flux_x  :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(vol_flux_y   :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(mass_flux_y  :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   IN(fields,depth,chunk_neighbours) &
!DIR$   IN(CHUNK_BOTTOM,EXTERNAL_FACE,CHUNK_TOP,CHUNK_LEFT,CHUNK_RIGHT) &
!DIR$   IN(FIELD_DENSITY0,FIELD_DENSITY1,FIELD_ENERGY0,FIELD_ENERGY1,FIELD_PRESSURE) &
!DIR$   IN(FIELD_VISCOSITY,FIELD_SOUNDSPEED,FIELD_XVEL0,FIELD_XVEL1,FIELD_YVEL0) &
!DIR$   IN(FIELD_YVEL1,FIELD_VOL_FLUX_X,FIELD_VOL_FLUX_Y,FIELD_MASS_FLUX_X) &
!DIR$   IN(FIELD_MASS_FLUX_Y,NUM_FIELDS)
