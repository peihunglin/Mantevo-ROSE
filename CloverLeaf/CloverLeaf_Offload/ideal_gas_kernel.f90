...
SUBROUTINE ideal_gas_kernel(g_mic_device,x_min,x_max,y_min,y_max,                &
                            density,                                &
                            energy,                                 &
                            pressure,                               &
                            soundspeed                              )                              
...
!DIR$   in(density     :length(0) alloc_if(.false.) free_if(.false.)) &
!DIR$   in(energy      :length(0) alloc_if(.false.) free_if(.false.)) &
